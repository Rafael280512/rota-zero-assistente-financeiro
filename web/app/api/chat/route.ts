...
function num(v: any): number {
        ...
        if (typeof v === "string") { const n = parseFloat(v.replace(/[^0-9.,-]/g, "").replace(".", "").replace(",", ".")); return isNaN(n) ? 0 : n; }
        return 0;
}

function normalizarPerfil(p: any): PerfilUsuario | null {
        if (!p || typeof p !== "object") return null;
        const out: any = {};
        if (p.nome) out.nome = String(p.nome);
        if (p.rendaMensal != null || p.renda != null) out.rendaMensal = num(p.rendaMensal ?? p.renda);
        if (p.temDividas != null) out.temDividas = !!p.temDividas;
        if (p.totalDividas != null || p.dividas != null) out.totalDividas = num(p.totalDividas ?? p.dividas);
        if (Array.isArray(p.gastos)) {
                  out.gastos = p.gastos.map(function (g: any) {
                              return { categoria: String(g.categoria || g.nome || g.descricao || "Outros"), valor: num(g.valor ?? g.valorMensal ?? g.total) };
                  }).filter(function (g: any) { return g.valor > 0; });
        }
        if (Array.isArray(p.metas)) {
                  out.metas = p.metas.map(function (m: any) {
                              return { nome: String(m.nome || m.descricao || m.titulo || m.objetivo || "Meta"), alvo: num(m.alvo ?? m.valor ?? m.meta ?? m.total), atual: num(m.atual ?? m.acumulado ?? m.jaGuardado ?? 0) };
                  }).filter(function (m: any) { return m.alvo > 0; });
        }
        if (p.fase) out.fase = String(p.fase);
        return out;
}
...
export async function POST(req: NextRequest) {
        try {
                  ...
                  if (!resp.ok) {
                              ...
                              const amigavel = resp.status === 503 ? "O assistente esta com muita demanda no momento. Tente novamente em instantes." : "Erro ao falar com o Rota Zero. Tente novamente.";
                              return NextResponse.json({ error: amigavel, detalhe: t.slice(0, 200) }, { status: 502 });
                  }
                  ...
                  if (m) { try { perfilAtualizado = normalizarPerfil(JSON.parse(m[1])); } catch (e) { perfilAtualizado = null; } }
